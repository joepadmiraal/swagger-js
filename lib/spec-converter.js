...
SwaggerSpecConverter.prototype.responseMessages = function(operation, existingOperation) {
  ...
  var isArray = false;
  if (existingOperation.type === 'array') {
    isArray = true;
  }
  ...
  if(Array.isArray(existingOperation.responseMessages)) {
    ...
    for(i = 0; i < existingResponses.length; i++) {
      ...
      } else if (isArray && existingResponse.code === 200) {
        response.schema = {
          type: 'array',
          items: {'$ref': '#/definitions/' + existingOperation.items.$ref}
        };
      }
...
SwaggerSpecConverter.prototype.toJsonSchema = function(source) {
  ...
  if(lcType.indexOf('list[') === 0) {
    ...
    // Hack to prevent the json editor from swagger ui to fail
    {return {type: 'object'};}
  } else if(lcType === 'object') {
    // Hack to prevent the json editor from swagger ui to fail
    {return {type: 'object'};}
  } else {
